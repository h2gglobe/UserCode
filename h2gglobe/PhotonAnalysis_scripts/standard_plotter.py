...
nhists = len(file_names[file_names.keys()[0]])-2
...
for i in range(2,nhists+2):
	...
	for l in range(len(linked_names)):
	  ...
	  if dont_stack:	     
	      tmp[l].SetLineColor(titles_colors[cc][1])
	      tmp[l].SetLineWidth(2)	
	      if  tmp[l].Integral() != 0: tmp[l].Scale(1./tmp[l].Integral())
	      leg.AddEntry(tmp[l],titles_colors[cc][0],'L')
	  else:
	      tmp[l].SetFillColor(titles_colors[cc][1])
	      leg.AddEntry(tmp[l],titles_colors[cc][0],'F')
	...
	if plot_ratio:
	  ...
	  hist_rat.Draw('same')
        # ----------------------------------------------------
	if plot_as_gif: c.SaveAs(plottingdir+'/'+data_hist.GetName()+'.gif')
	else: c.SaveAs(plottingdir+'/'+data_hist.GetName()+'.pdf')
